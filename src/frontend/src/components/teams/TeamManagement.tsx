...
export const TeamManagement: React.FC<TeamManagementProps> = ({
  ...
  const handleCreateTeamSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!currentUser || currentUser.user_type !== 'organizer') return;
    try {
  ...
  const isUserTeamManager = currentTeamDetails?.user_role === 'manager' || currentUser?.user_type === 'organizer';
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Collaborate in teams, organize shared documentation into projects, and manage template access.
          </p>
        </div>
        ...
        <div className="flex items-center space-x-2.5">
          <button
            ...
            className="flex items-center space-x-1.5 px-3.5 py-2 text-sm font-medium text-slate-700 bg-white border border-slate-300 hover:bg-slate-50 rounded-xl shadow-xs transition-colors"
          >
          ...
          {currentUser?.user_type === 'organizer' ? (
            <button
              ...
              className="flex items-center space-x-1.5 px-4 py-2 text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl shadow-sm transition-colors"
            >
            ...
            <button
              onClick={() => setIsCreateTeamOpen(true)}
              className="flex items-center space-x-1.5 px-4 py-2 text-sm font-medium text-slate-600 bg-slate-100 hover:bg-slate-200 border border-slate-300 rounded-xl transition-colors"
            ...
            {teams.length === 0 ? (
              <div className="text-center py-8 px-4 bg-slate-50 rounded-xl border border-dashed border-slate-200">
                ...
                <p className="text-xs text-slate-500 mt-1">
                  Join a team using a join code or switch to Organizer to create one.
                </p>
              </div>
            ) : (
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between mb-1">
                        ...
                        >
                          {t.user_role === 'manager' ? 'Manager' : 'Member'}
                        </span>
                      </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      >
                        You are {currentTeamDetails.user_role === 'manager' ? 'Manager (Can edit templates)' : 'Member'}
                      </span>
                    </div>
                      ...
                      {currentTeamDetails.members?.map((member) => {
                        ...
                        return (
                          <div key={member.user_id} className="p-4 flex items-center justify-between hover:bg-slate-50/50">
                            <div className="flex items-center space-x-3">
                              ...
                              <div>
                                <div className="flex items-center gap-2">
                                  ...
                                  >
                                    {member.role === 'manager' ? <ShieldCheck className="w-3 h-3" /> : <Users className="w-3 h-3" />}
                                    {member.role === 'manager' ? 'Manager (Can Edit Templates)' : 'Member'}
                                  </span>
                                </div>
                            ...
                            {isUserTeamManager && !isSelf && (
                              <div className="flex items-center space-x-2">
                                <button
                                  onClick={() => handleToggleMemberRole(member)}
                                  className="text-xs font-semibold text-slate-600 hover:text-indigo-600 bg-slate-100 hover:bg-indigo-50 px-2.5 py-1 rounded-lg transition-colors"
            ...
            {currentUser?.user_type !== 'organizer' ? (
              <div className="space-y-4">
                <div className="flex items-center space-x-2 text-amber-600">
                  ...
                  <h3 className="text-base font-bold text-slate-900">Organizer Role Required</h3>
                </div>
                <p className="text-xs text-slate-600 leading-relaxed">
                  Only users with the <strong>Organizer</strong> user type can create new teams. Regular users can join existing teams with a token.
                </p>
                <div className="p-3 bg-indigo-50 border border-indigo-100 rounded-xl">
                  <p className="text-xs text-indigo-900 font-medium">
                    You can switch your role to <strong>Organizer</strong> instantly in your Account Settings!
                  </p>
                </div>
