...
graph TD
    ...
    subgraph Host["Host Machine (macOS / Linux / Windows Server)"]
        ...
        DevUser["Developer Direct Access<br/>(http://localhost:3000)"]
        
        subgraph DockerCompose["Docker Compose Virtual Network (app-net)"]
            ContainerNginx["Frontend Service (web_frontend)<br/>Container: Nginx + ReactJS<br/>Host Port: 3000 -> Container Port: 80<br/>- Serves compiled React SPA assets<br/>- Proxies /api/ to backend"]
    ...
    HostNginx -->|proxy_pass :3000| ContainerNginx
    DevUser -->|Direct HTTP :3000| ContainerNginx
...
- **Developer & Host Access**:
  - The developer can directly access the container's Nginx on host port `3000` (`http://localhost:3000`).
...
2. **Production Host Nginx Access**:
   ...
   - Host Nginx forwards to `http://127.0.0.1:3000`.
